...
def find_connected_joints(param, paf_upsamp, joint_list_per_joint_type, limbSeq, num_intermed_pts=10):
    """
            ...
            for i, joint_src in enumerate(joints_src):
                ...
                for j, joint_dst in enumerate(joints_dst):
                    ...
                    criterion1 = (np.count_nonzero(
                        ...
                        # score_intermed_pts > param['thre2']) > 0.5 * num_intermed_pts)
...
def group_limbs_of_same_person(connected_limbs, joint_list, limbSeq):
    """
    ...
    # print(connected_limbs[0].shape)
    # print([pair[0, :2].astype(int).tolist() for pair in connected_limbs if len(pair)>0])
    # exit()
    # print([pair[:2] for pair in connected_limbs])
    for limb_type in range(len(limbSeq)):
        joint_src_type, joint_dst_type = limbSeq[limb_type]
...
def plot_pose(img_orig, joint_list, person_to_joint_assoc, limbSeq, bool_fast_plot=True, plot_ear_to_shoulder=False):
...
def decode_pose(img_orig, param, heatmaps, pafs, limbSeq):
    ...
    connected_limbs = find_connected_joints(param,
                                            paf_upsamp, joint_list_per_joint_type, limbSeq)


    # Step 3: associate limbs that belong to the same person
    person_to_joint_assoc = group_limbs_of_same_person(
        connected_limbs, joint_list, limbSeq)


    # (Step 4): plot results
    to_plot, canvas = plot_pose(img_orig, joint_list, person_to_joint_assoc, limbSeq)
